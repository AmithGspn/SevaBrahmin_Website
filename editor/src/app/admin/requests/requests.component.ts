...
export class RequestsComponent implements OnInit {
    ...
    this.appService.getRequests().subscribe((data:any) => {
      ...
      for (let request of data) {
        ...
        if (request.handledBy) {
          this.appService.getVolunteerById(request.handledBy).subscribe((volunteer: any) => {
            if (volunteer.length > 0) {
              request["volName"] = volunteer[0].firstName;
            }
  ...
  viewDescription(request,row_index) {
    this.rowIndex = row_index;
    let Table = document.getElementById("myTable");
    let rows = Table.getElementsByTagName("tr");
    rows[row_index+1].setAttribute("style","background-color:  black; color: white");
  ...
  //   let Table = document.getElementById("myTable");
  //   let rows = Table.getElementsByTagName("tr");
  //   rows[i+1].setAttribute("style","cursor: pointer"); 
  // }
  ...
  onClickAssign(volunteer) {
    ...
    // this.volunteers = [];
    // this.Request = [];
    // this.appService.getRequestsById(this.recipientId).subscribe(async(data:any) => {
    // await this.Request.push(data[0]);
    // })
    console.log(this.Request, "aaaaaaaaaaaaaaaaaaaa");
    // this.appService.getVolunteer().subscribe(async(data:any) => {
    //   console.log(data);
    //   for(let volunteer of data) {
    ...
    let REQUESTS = {
      requestType: this.Request[0].requestType ,
      request_id: this.Request[0].request_id
    }
    volunteer.requests.push(REQUESTS);
    this.volunteerName = volunteer.firstName;
    this.Request[0].donor = "None";
    this.Request[0].status = "Processing";
    this.Request[0].handledBy = this.volunteer_id,
    ...
    this.appService.putRequests(this.Request[0]).subscribe((data:any) => {
      console.log(data)
    })
    
      // let formData = {
      //   recipientName:this.Request[0].recipientName,
      //   familyName: this.Request[0].familyName,
      //   country: this.Request[0].country,
      //   state: this.Request[0].state,
      //   city: this.Request[0].city,
      //   email: this.Request[0].email,
      //   contact: this.Request[0].contact,
      //   occupation: this.Request[0].occupation,
      //   requestType: this.Request[0].requestType,
      //   amount: this.Request[0].amount,
      //   description: this.Request[0].description,
      //   status: "Processing",
      //   handledBy: this.volunteer_id,
      //   donor: "None",
      //   request_id: this.Request[0].request_id,
      //   recipient_id: this.Request[0].recipient_id
      // }
      
        // this.appService.getVolunteerById(this.volunteer_id).subscribe((data:any) => {
          // data[0].requests.push(REQUESTS);
          // data[0].requestType = this.selectedRequest[0].requestType;
          // this.appService.putVolunteer(data[0]).subscribe((data) => {
      document.querySelector('.popup-model').setAttribute("style","display:none;");
      this.onAssigned(this.Request[0]);
            window.location.reload();
        // })
      // })
    }
  ...
  onSubmit1() {
    ...
    // console.log(state)
    // console.log(city)
    let result = this.searchVolunteer;

    this.searchVolunteer = result.filter(v => 
      v.state === state
    )

    // this.searchVolunteer = result.filter(volunteerStateSearch)
    // if(this.city == "") {
    //   this.searchVolunteer = this.searchVolunteer.filter(function(item) {
    ...
    //     if (item[key] === undefined || item[key] != location[key])
    //       return false;
    //   }
    //   return true;
    // });

    // this.searchVolunteer = this.volunteers.filter(singleItem => singleItem.state.includes(this.state) && singleItem.city.includes(this.city));
